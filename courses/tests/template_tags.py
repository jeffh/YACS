...
class RemoveZeroPrefixTest(TestCase):
    def test_remove_zero_prefix_with_zero_prefix(self):
        self.assertEqual(tags.remove_zero_prefix('01'), '1')

    def test_remove_zero_prefix_with_no_zero_prefix(self):
        self.assertEqual(tags.remove_zero_prefix('21'), '21')
...
class GetTest(TestCase):
    def test_get_with_object(self):
        obj = Mock()
        obj.lol = 342
        self.assertEqual(tags.get(obj, 'lol'), 342)

    def test_get_with_index(self):
        obj = [1, 2, 3]
        self.assertEqual(tags.get(obj, 1), 2)

    def test_get_with_key_index(self):
        obj = {'lol': 'rofl'}
        self.assertEqual(tags.get(obj, 'lol'), 'rofl')

    def test_get_should_return_None_on_invalid_access(self):
        obj = []
        self.assertEqual(tags.get(obj, 4), None)
...
class DowShortTest(TestCase):
    def test_monday(self):
        self.assertEqual(tags.dow_short('Monday'), 'Mo')

    def test_tuesday(self):
        self.assertEqual(tags.dow_short('Tuesday'), 'Tu')

    def test_wednesday(self):
        self.assertEqual(tags.dow_short('Wednesday'), 'We')

    def test_thursday(self):
        self.assertEqual(tags.dow_short('Thursday'), 'Th')

    def test_friday(self):
        self.assertEqual(tags.dow_short('Friday'), 'Fr')

    def test_saturday(self):
        self.assertEqual(tags.dow_short('Saturday'), 'Sa')

    def test_sunday(self):
        self.assertEqual(tags.dow_short('Sunday'), 'Su')

    def test_return_None_otherwise(self):
        self.assertEqual(tags.dow_short('foobar'), None)

    def test_convert_list(self):
        collection = ['Monday', 'Wednesday', 'Thursday']
        self.assertEqual(tags.dow_short(collection), ('Mo', 'We', 'Th'))
